...
DEFINE_SIMPLE_ATTRIBUTE(ap1302_sipm_addr_fops, ap1302_sipm_addr_get,
			 ap1302_sipm_addr_set, "0x%08llx\n");
DEFINE_SIMPLE_ATTRIBUTE(ap1302_sipm_data_fops, ap1302_sipm_data_get,
			 ap1302_sipm_data_set, "0x%08llx\n");


static int ap1302_basic_addr_get(void *arg, u64 *val)
{
	struct ap1302_device *ap1302 = arg;

	mutex_lock(&ap1302->debugfs.lock);
	*val = ap1302->debugfs.basic_addr;
	mutex_unlock(&ap1302->debugfs.lock);

	return 0;
}

static int ap1302_basic_addr_set(void *arg, u64 val)
{
	struct ap1302_device *ap1302 = arg;

	mutex_lock(&ap1302->debugfs.lock);
	ap1302->debugfs.basic_addr = val;
	mutex_unlock(&ap1302->debugfs.lock);

	return 0;
}

static int ap1302_basic_data_get(void *arg, u64 *val)
{
	struct ap1302_device *ap1302 = arg;
	u32 value;
	u32 addr;
	...
	addr = ap1302->debugfs.basic_addr;
	if (!addr) {
		ret = -EINVAL;
		goto unlock;
	}
...
static int ap1302_basic_data_set(void *arg, u64 val)
{
	struct ap1302_device *ap1302 = arg;
	u32 addr;
	int ret;
...
DEFINE_SIMPLE_ATTRIBUTE(ap1302_basic_addr_fops, ap1302_basic_addr_get,
			 ap1302_basic_addr_set, "0x%08llx\n");
DEFINE_SIMPLE_ATTRIBUTE(ap1302_basic_data_fops, ap1302_basic_data_get,
			 ap1302_basic_data_set, "0x%08llx\n");


static int ap1302_log_status_get(void *arg, u64 *val)
{
	struct ap1302_device *ap1302 = arg;
	int ret = 0;

	ap1302_log_status(ap1302);

	return ret;
}

DEFINE_SIMPLE_ATTRIBUTE(ap1302_log_status_fops, ap1302_log_status_get,
			 NULL, "%llu\n");
...
static void ap1302_debugfs_init(struct ap1302_device *ap1302)
{
	...
	debugfs_create_file("sipm_addr", 0600, ap1302->debugfs.dir,
				   ap1302, &ap1302_sipm_addr_fops);
	debugfs_create_file("sipm_data", 0600, ap1302->debugfs.dir,
				   ap1302, &ap1302_sipm_data_fops);

	debugfs_create_file("basic_addr", 0600, ap1302->debugfs.dir,
				   ap1302, &ap1302_basic_addr_fops);
	debugfs_create_file("basic_data", 0600, ap1302->debugfs.dir,
				   ap1302, &ap1302_basic_data_fops);

	debugfs_create_file("log_status", 0400, ap1302->debugfs.dir,
				   ap1302, &ap1302_log_status_fops);
}
...
//		container_of(ctrl->handler, struct ap1302_device, ctrls);
	...
	case V4L2_CID_COLORFX:
		return ap1302_set_special_effect(ap1302, ctrl->value);

	case V4L2_CID_SCENE_MODE:
		return ap1302_set_scene_mode(ap1302, ctrl->value);
...
static int ap1302_hw_init(struct ap1302_device *ap1302)
{
	...
	for (retries = 0; retries < MAX_FW_LOAD_RETRIES; ++retries) {
		...
		if (ret != -EAGAIN) {
			dev_err(ap1302->dev, "Firmware load error\n");
			goto error_power;
		}
...
//		return PTR_ERR(ap1302->standby_gpio);
	...
	ret = of_property_read_u32(ap1302->dev->of_node, "csi_id",
					&(ap1302->sdata.csi));
	if (ret) {
		dev_err(ap1302->dev, "csi id missing or invalid\n");
	//	return ret;
	}
	...
	ap1302->width_factor = 1; //num_sensors;
...
*/
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if (tgt_fps > 50) {
			timeperframe->denominator = 50; /* MAX_FPS */
			timeperframe->numerator = 1;
		} else if (tgt_fps < 10) {
			timeperframe->denominator = 10; /* MIN_FPS */
			timeperframe->numerator = 1;
		}
#if 0
...
static int ap1302_ioctl_enum_fmt_cap(struct v4l2_int_device *s,
...
static int ap1302_ioctl_g_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	struct ap1302_device *ap1302 = s->priv;
...
static int ap1302_ioctl_g_chip_ident(struct v4l2_int_device *s, int *id)
{
	((struct v4l2_dbg_chip_ident *)id)->match.type =
					V4L2_CHIP_MATCH_I2C_DRIVER;
	strcpy(((struct v4l2_dbg_chip_ident *)id)->match.name,
		"ap1302_ar1335_camera");

	return 0;
}

static int ap1302_ioctl_dev_exit(struct v4l2_int_device *s)
{
	void *mipi_csi2_info;
	int ret;
	struct ap1302_device *ap1302 = s->priv;
...
static struct v4l2_int_slave ap1302_slave = {
	.ioctls = ap1302_ioctl_desc,
	.num_ioctls = ARRAY_SIZE(ap1302_ioctl_desc),
};

static struct v4l2_int_device ap1302_int_device = {
	.module = THIS_MODULE,
	.name = "ap1302_ar1335",
...
static int ap1302_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	memset(&(ap1302->sdata), 0, sizeof(struct sensor_data));
...
//	ret = ap1302_config_v4l2(ap1302);
	ret = v4l2_int_device_register(&ap1302_int_device);

	if (ret)
		goto error_hw_cleanup;

	dev_info(ap1302->dev, "AP1302+AR1355 initialized correctly\n");
